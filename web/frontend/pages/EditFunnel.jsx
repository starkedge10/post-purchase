...
async function getData(){
    ...
    try {
        ...
        const accept_pro = data[0].accept_product_id;
        const decline_pro = data[0].decline_product_id;
        const main_pro = data[0].main_product_id;
        const funnel_title = data[0].funnel_name;
        const funnel_status = data[0].funnel_status;
        
        setPublish(funnel_status);

        const funnel_trigger = data[0].funnel_trigger;
        ...
        const funnel_details = {
            ...
            decline: decline_pro,
            main: main_pro,
            funl_name: funnel_title,
            funl_status: funnel_status,
            funl_trigger: funnel_trigger
        };

        for (let i = 0; i < data.length; i++) {
            const pro = data[i];
            const pro_title = pro.title;
            const pro_price = pro.price;
            const pro_image = pro.image;
            products[pro.product_id] = { title: pro_title, price: pro_price, image: pro_image };
        }

        const funel_main_id = funnel_details.main;
        const pro_title = products[funel_main_id]?.title;
